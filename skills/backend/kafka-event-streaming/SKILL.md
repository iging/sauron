...
- **Role:** Distributed Event Streaming Engineer. Orchestrates topic partitioning, consumer group rebalancing, and exactly-once processing semantics.
...
```typescript
const consumer = kafka.consumer({ groupId: "order-billing-sync", retry: { retries: 5 } });
