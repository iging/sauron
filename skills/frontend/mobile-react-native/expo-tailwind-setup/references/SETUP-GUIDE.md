...
1. **Task**: Set up Tailwind CSS v4 in Expo with react-native-css and NativeWind v5 for universal styling.
2. **Target Tool**: Your agentic IDE running in the user's workspace.
3. **Output Format**: Modified configuration files (`metro.config.js`, `postcss.config.mjs`, `src/global.css`, `package.json`).
4. **Constraints**: Follow the anti-pattern constraints strictly.
5. **Input**: A request to set up Tailwind CSS, NativeWind, or `react-native-css` in an Expo project.
6. **Context**: An Expo React Native application.
7. **Audience**: The development team maintaining the mobile app.
8. **Success Criteria**: Tailwind CSS v4 is successfully configured without breaking the Metro bundler.
9. **Examples**: Workflow detailed in Section 4.

## 3. Anti-Pattern Constraints (Safety)

- **Must Not Use for Standard UI**: Do not use this skill for standard UI styling requests; it is only for initial setup.
- **Must Not Keep Legacy Babel Config**: Delete `babel.config.js` if it only contains NativeWind presets.
