...
- Patches applied — layout-aware check (hoisted, pnpm isolated, and workspace layouts; run at the workspace root):
  ...
  `find node_modules -path '*metro-runtime/src/skills/HMRClient.js' -exec grep -l __workletsModuleProxy {} +` each match at
...
- **metro-runtime** (`src/skills/HMRClient.js`): calls `global.__workletsModuleProxy?.propagateModuleUpdate(...)` on HMR inject
...
find node_modules -path '*metro-runtime/src/skills/HMRClient.js' -exec grep -l __workletsModuleProxy {} +    # expect >=1 file
```
