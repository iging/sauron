...
> "When we align the interface to the way we think and move, something magical happens — it stops feeling like a computer and starts feeling like a smooth extension of us."
...
- **Feedback must be continuous _during_ the interaction, not at the end.** For a drag, slider, or drawer, update the UI 1:1 with the pointer the whole way through — never animate only when the gesture completes.
...
- Track a short **velocity/position history** (last few `pointermove` events), not the current point — you'll need velocity at release.
...
A pre-scripted, fixed-duration animation can't respond to new input. A spring can — new input changes the target, and the motion stays continuous. Reach for springs for anything a user can touch.
...
- Add bounce (**damping ~`0.8`**) **only when the gesture itself carried momentum** (a flick, a throw, a drag release). Overshoot on a menu that faded in feels wrong; overshoot on a card you flicked feels right.
...
Humans predict a final state from a trajectory. Intermediate motion should telegraph where things are going — Control Center modules "grow up and out toward your finger." Make the in-between frames point at the outcome, not interpolate blindly to it.
...
Smoothness is about _what's in the frames_, not the frame rate.
...
- **Scroll edge effects, not hard dividers.** Instead of a 1px border under a sticky header, fade a small blur/gradient mask where content meets floating chrome — only where floating UI overlaps content.
- **Materialize, don't fade.** For glass/blur surfaces, animate blur radius and scale together on enter/exit, so the surface reads as a real material arriving rather than a plain opacity fade.
