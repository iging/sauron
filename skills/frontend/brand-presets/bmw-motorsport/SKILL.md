...
department: frontend
ownerAgent: aragorn
triggerCommand: /brand-bmw-motorsport
...
- **Carbon Gray** (`{colors.carbon-gray}`  -  #2b2b2b): Carbon-fiber surface tone used on technical-spec cards.
...
**`category-tab`** + **`category-tab-active`**  -  The category selector tabs used on the magazine and topics pages (for example, "ALL  MAGAZINE  MODELS  LIFESTYLE  MOTORSPORT"). Tabs render as text-only labels in `{typography.label-uppercase}`. Active state changes text color from `{colors.body}` to `{colors.on-dark}` and adds a 2px white underline below the label. No background fill, no rounded corners.
...
|---|---|---|
| Mobile | < 768px | Hamburger nav; hero h1 scales 8048px; demo grid 1-up; photo cards stack full-width; footer 4 cols  1 |
