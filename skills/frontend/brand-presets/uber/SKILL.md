---
name: brand-uber
description: An interpretation of Uber's design language  -  a transportation-and-delivery super-app brand whose web surface is a black-and-white duet, framed by a custom geometric display sans, accented by a single signature pill shape (radius 999px) on every interactive element, and decorated only by editorial 4:3 illustrations of riders, drivers, and city objects.
department: frontend
ownerAgent: aragorn
triggerCommand: /brand-uber
...
**`button-subtle`**  -  the gray secondary pill used for tertiary actions inside cards (for example, "Learn more" / "Use Reserve").
...
**`card-soft-tinted`**  -  the gray-tinted card used as a sub-region inside the page (for example, "Plan for later" callout).
...
**`showcase-image-card`**  -  the giant "GOGET 2026" annual showcase card.
...
- Background `{colors.canvas}` on light pages, switches to `{colors.ink}` on the rare dark page (for example, Uber Eats hero). Padding `{spacing.lg} {spacing.3xl}`.
