...
### Text
- **Ink** ({colors.ink}): All headline, body, and caption type on light surfaces. There is no softer mid-gray text role on marketing  -  body copy is always black at weight 320340, and weight (not opacity) carries the hierarchy.
...
- **Tight line-heights on display, generous on body.** Display sizes run 1.001.10; body runs 1.401.45. The contrast reinforces that headlines are graphics and body copy is for reading.
...
Figma's marketing system is shadow-light by design  -  the color blocks substitute for traditional elevation. Where most SaaS sites use a shadowed white card to draw attention, Figma uses a saturated background panel. This makes the rare actual shadow (for example, a floating template card hovering over a cream section) feel like an exception worth noticing.
...
- Reserve `{colors.primary}` for genuine primary CTAs and selected states (for example, `pricing-tab-selected`). Don't use it as a decorative accent.
...
- Don't introduce new accent colors outside the documented `{colors.block-*}` palette and `{colors.accent-magenta}`. Adding, for example, a saturated brand orange would break the system.
...
1. Focus on ONE component at a time and reference it by its `components:` token name (for example, `{components.button-primary}`, `{components.color-block-section}`).
