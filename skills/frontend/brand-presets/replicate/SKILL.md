...
department: frontend
ownerAgent: aragorn
triggerCommand: /brand-replicate
...
- The single most important action on a page (for example, "Sign in with GitHub", "Try a model").
...
- Background `{colors.surface-card}`, label `{colors.ink}`, 1px solid `{colors.hairline}`, `rounded: {rounded.full}`, 3636px circular.
...
- Pill chips set in a horizontal row above content (for example, "All", "Featured", "Image generation", "Audio"), `{component.sub-nav-pill}` styling.
...
- Background `{colors.surface-card}` placeholder behind 1:1 photography, `rounded: {rounded.full}`, 4040px (32px in metadata contexts).
...
### Image Behavior
- Model thumbnails serve at 1.5 and 2 DPR; below 768px the system swaps to a 600600 export instead of 12001200.
