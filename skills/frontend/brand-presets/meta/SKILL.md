...
**`button-icon-circular`**  -  4040px circular utility buttons (carousel arrows, share, favorite).
...
- Left: Meta wordmark logo (6114px). Center: pill-tab category nav. Right: search-pill + circular icon buttons (account, cart).
...
- Thumbnails: 8080px, `{rounded.lg}`, `{colors.surface-soft}` background, 1px `{colors.hairline-soft}` border (active border switches to `{colors.ink-deep}`).
- Main image area: ~720720px on desktop, `{rounded.xxxl}` rounding, photographic surface.
...
- Circular icon buttons are 4040px  -  at the AA floor; bumps to 4444px on mobile via override.
- Color swatch circles are 3232px. To hit AAA, the swatch carries a 12px clear hit zone around it (effective hit target ~56px).
...
- **Feature grids** (3-up, 4-up) collapse to 2-up at 7681023px and 1-up at < 768px. Card padding compresses from `{spacing.xxl}` to `{spacing.xl}` at the 1-up breakpoint.
...
- Animation/transition timings are not extracted; recommend 150250ms ease-out for primary surface transitions and 300ms ease-in-out for accordion expand/collapse.
