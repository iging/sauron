---
name: brand-nintendo-retro-2001
...
department: frontend
ownerAgent: aragorn
triggerCommand: /brand-nintendo-retro-2001
...
| `{typography.hero-tagline}` | 15px | 700 | 1.3 | 0 | Hero supporting line (" graphics, great sound") |
...
- **Body**: a full-width hero panel, then a **two-column split**  -  a ~two-thirds content column of stacked panels (Official News list, Featured Sites 22 grid, and so forth. ) beside a ~one-third **right action rail** (Login / Subscribe / Newsletter / Help buttons, an info box, a side promo card).
...
#### Touch Targets
Not a consideration in the original (mouse-only era); buttons and chips are 1828px tall. A modern port must enlarge the `{components.button-primary}`, `{components.button-icon-arrow}`, and `{components.radio-option}` hit areas to a 4444px minimum.
...
#### Image Behavior
Hero fields are **full-bleed photographic plates** clipped to the panel's beveled rectangle; featured-site and game tiles are fixed-pixel thumbnails (~9560px) in a tight grid. No lazy loading (era predates it). Product renders sit on textured backdrops rather than transparent cutouts.
...
- A `{colors.carbon}`-framed `{rounded.sm}` tile (~9560px) holding a screenshot thumbnail with a `{typography.micro}` URL caption (for example, www.pokemon.com). Tight `{spacing.xxs}` padding, arranged in a 22 grid.
...
- A `{colors.lavender}` `{rounded.sm}` card carrying an outlined product wordmark (for example, Game Boy Advance) over a small product render, in the right rail. `{spacing.md}` padding.

**`system-tile`**  -  Hardware grid tile (Systems page)
- A `{colors.periwinkle}` `{rounded.sm}` tile holding a console render over a circuit-board backdrop with an outlined system name (Nintendo 64, Super Nintendo, Game Boy). `{spacing.sm}` padding, arranged in a 23 grid.
...
- A white `{rounded.sm}` grid calendar in `{typography.micro}`, with a `{colors.carbon}` caption header (for example, "June 01"), highlighted event dates, and a month-select dropdown. `{spacing.sm}` padding.
