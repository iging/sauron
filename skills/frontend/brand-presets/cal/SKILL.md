...
- Nav-pill-group (`{component.nav-pill-group}`)  -  a small pill-radius wrapper around grouped nav segments (for example, the sub-nav switcher between product views). The pill wrapper is one of the system's signature interactive components.
...
- Avatar circles in testimonial sections sometimes carry pastel fill colors (`{colors.badge-orange}`, `{colors.badge-pink}`, and so forth. )  -  adds a small chromatic flourish without breaking the monochrome brand voice.
...
Avatar photos use `{rounded.full}` (perfect circles) at 36px or 40px. Product UI fragments inside marketing cards retain their native chrome (which often has its own internal radii  -  for example, calendar grid cells, button rows). Hero illustration zones use 16:9 or 4:3 ratios with `{rounded.xl}` corners.
...
**`badge-pill`**  -  Small pill label used for category tags ("Product", "Article", "New") and pastel-fill avatar substitutes. Background `{colors.surface-card}` or one of the badge pastels (`{colors.badge-orange}`, `{colors.badge-pink}`, and so forth. ), text `{colors.ink}`, type `{typography.caption}` (13px / 500), rounded `{rounded.pill}`, padding 4px  12px.
...
|---|---|---|
| Mobile | < 768px | Hamburger nav; hero h1 6432px; hero-app-mockup-card stacks below content; feature grids 1-up; pricing 1-up; footer 4 cols  1 |
| Tablet | 7681024px | Top nav stays horizontal but tightens; nav-pill-group wraps; feature cards 2-up; pricing 2-up |
| Desktop | 10241440px | Full top-nav with all menu items; 3-up feature cards; 4-up pricing tiers |
