...
The page rhythm is monolithic: dark canvas top to bottom with subtle elevation steps via card surfaces. The brand's strongest visual signature is a **four-pane terminal-style mockup**  -  a 22 grid of dark code/output panels with a central blue spotlight glow behind them.
...
- Terminal-mockup hero: 22 grid of code/output panes is the brand signature.
...
- Terminal mockup grid: 22 equal-size panes.
...
- **Terminal-pane brightness ladder**  -  22 mockup uses canvas-deep outer + surface-card-elevated panes.
...
**`terminal-mockup-grid`**  -  The brand's strongest visual signature. 22 grid of dark code/output panels inside a `{rounded.xl}` (16px) container. Background `{colors.canvas-deep}`, padding 32px, gap 16px.
...
**`toolkit-card`**  -  4-up toolkit grid (Slack, GitHub, Stripe, Notion, Linear, and so forth. ). Background `{colors.surface-card}`, text `{colors.body-strong}`, type `{typography.title-sm}`, rounded `{rounded.lg}`, padding 20px. 40px square `{component.toolkit-icon}` top, toolkit name, one-line description.
...
**`footer-dark`**  -  Closing footer. Background `{colors.canvas}`, text `{colors.body}`. 5-column link list. 6448px padding.
...
- Use the 22 terminal-mockup grid as the homepage hero anchor.
...
|---|---|---|
| Mobile | < 640px | Hero h1 7236px; terminal mockup grid collapses to single pane; toolkit grid 1-up; nav hamburger. |
| Tablet | 6401024px | Hero h1 56px; terminal mockup grid stays 22; toolkit grid 2-up. |
| Desktop | 10241280px | Full hero h1 72px; full 22 terminal mockup; toolkit grid 4-up. |
...
- Terminal mockup 22 grid collapses to a single pane on mobile.
