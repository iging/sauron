...
|---|---|---|---|---|---|
| `{typography.display-xl}` | 56px | 700 | 0.95 | 0 | Hero headlines, campaign titles ("E-TECH ELEKTRKL", "REVOLUTION"). |
...
| 2  -  colour-blocked elevation | Surface colour shift (for example, `{colors.canvas}` card sitting inside a `{colors.surface-soft}` band) | Configurator detail cards, related-content rows. |
| 3  -  dark inversion | Card swaps to `{colors.surface-dark}` against a `{colors.canvas}` band | "Ticari ara" hero promo tiles, lifestyle storytelling cards. |

Drop shadows are extracted from the system but rarely visible on the marketing
surfaces. When they appear, they are very subtle (~10% opacity, 24px blur)
...
- The single most important action on a page (for example, "Hemen randevu al", "Hesapla", "Konfigratr balat").
...
- Tertiary action; appears alongside primary/secondary for "Detaylar gr", "Modeller", and so forth. **`button-outline-light`**  -  outlined CTA on dark
...
- The only place the system uses a pill  -  for top-level filter chips ("Servis & randevu", "Sahiplik dnemi geii", "Kampanyalar") and configurator tab switches.

**`button-icon-square`**  -  small icon button
- Background `{colors.canvas}`, 1px solid `{colors.hairline-strong}`, `rounded: {rounded.xs}`, 4040px square.
...
- Used in the homepage 2-up grid for "Hybrid ara modelleri", "binek ara sat kampanyalar" tiles.
...
- Lifestyle / commercial-vehicle storytelling tiles ("ticari ara sat kampanyalar").
...
- "SCENIC E-TECH ELEKTRKL" hero on the homepage.
...
- Right-side scrolling list on the configurator: "kasa tipi", "motor seimi", "versiyon seimi", "renk seenekleri", and so forth. **`configurator-swatch`**  -  circular colour pick
- Background `{colors.surface-soft}` (or the actual car colour), `rounded: {rounded.full}`, 5656px.
...
- Left: diamond logomark. Centre: top-level nav ("Modeller", "Hizmetler", "Renault Yaam", "MyRenault"). Right: language switcher + login icon.
...
- Pill chips set in a horizontal scroll bar between hero and content body (for example, "Servis & randevu", "Sahiplik dnemi geii", "Kampanyalar"), `{component.button-pill}` styling.
...
### Touch Targets
- All buttons ship at minimum 4444px on mobile; default `{component.button-primary}` is 48px tall, comfortably exceeding WCAG AAA.
