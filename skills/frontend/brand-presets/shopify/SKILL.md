...
- Color `{colors.on-primary}`, no underline by default (links rely on context); for tertiary footer links, color shifts to one of the cool muted tones (`{colors.link-cool-1}` and so forth. ) with a persistent underline.
...
### Touch Targets
- Pill buttons hit  4444px on mobile via 12px vertical padding  16px line-height. WCAG AAA compliant.
