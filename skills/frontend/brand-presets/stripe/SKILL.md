...
Typography is built around **Sohne** at weight 300 with negative letter-spacing  -  the brand's editorial-density display signature. Display sizes (3256px) use -1.4px to -0.64px tracking; body sizes use 0; tabular caption sizes (where money and numerics matter) use the OpenType `tnum` feature plus a tightening -0.36 to -0.42px tracking. The `ss01` stylistic set is enabled across all roles.
...
- **Section padding**: 6496px on marketing surfaces; 3248px on dashboard / product surfaces.
...
- Background `{colors.canvas}`, text `{colors.ink-mute}`, type `{typography.caption}`, padding `{spacing.huge} {spacing.xl}` (64px 24px). Holds 46 columns of link groups, social icons, and a small legal row.
...
| Tablet | 7681023px | Pricing 2-up; dashboard composite simplifies to 2 panels |
| Mobile | < 768px | Pricing 1-up; hamburger nav; display drops 56  36px |

### Touch Targets
- Pill buttons hit  4040px on mobile via padding scaling. On smaller screens, buttons size up to 4444px to maintain WCAG AAA.
