...
**`ecosystem-tile`**  -  Square logo plate for ecosystem partner logos (TypeScript, React, Sentry, and so forth. ). Background `{colors.surface-card}`, rounded `{rounded.md}`, 64px size, 1px `{colors.hairline}` border.
...
**`footer-light`**  -  Closing white footer. Background `{colors.canvas}`, text `{colors.body}`. 5-column link list. 6448px padding.
...
|---|---|---|
| Mobile | < 640px | Hero h1 6432px; device mockup  single iPhone screen; feature grid 1-up; nav hamburger. |
