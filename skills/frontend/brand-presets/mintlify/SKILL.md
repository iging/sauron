...
**Geist Mono** (code): Monospace typeface used inside code blocks, inline code references, type signatures (for example, `string`, `number`, `boolean`), and property names in API documentation. Fallbacks: 'SF Mono', Menlo, Consolas, 'Geist Mono Fallback', monospace.
...
- **Documentation-grade body** (1.50 line-height on 1416px) ensures comfortable long-form reading in dense docs surfaces
...
- Logo walls use 6-up rows of customer logos at 80100px height each
...
**`button-icon-circular`**  -  3232px circular utility button (close, copy, arrow).
...
**`badge-type`**  -  Type signature chip in documentation (for example, `string`, `number`, `boolean`).
- Background `{colors.surface}`, text `{colors.steel}`, typography `{typography.code-sm}`, rounded `{rounded.sm}`, padding `2px 6px`.

**`badge-tag`**  -  Documentation tag chip (for example, `<Tabs>` reference highlighted in body text).
...
**`property-row`**  -  API property documentation row (for example, `defaultIndex` on the Tabs page).
...
**`sidebar-section-header`**  -  Uppercase section header inside sidebar (for example, "COMPONENTS", "PRIMITIVES").
...
### Touch Targets
- Pill buttons render at 3640px effective height  -  bumps to 44px on mobile via padding override
- Circular icon buttons: 3232px desktop  4444px mobile
...
- Animation/transition timings are not extracted; recommend 150200ms ease for hover/focus state transitions
...
- Code syntax highlighting palette inside docs is not formalized; documentation samples carry their own twoslash-style annotation system tokens (for example, `{colors.brand-tag}`, `{colors.brand-annotate}`, `{colors.brand-warn}`) but the full highlight scheme is not enumerated
